...
```{r mapamundo, echo=FALSE, message=FALSE, warning=FALSE, fig.align="left"}
...
ggplot() +
  geom_sf(data = mapamundi, alpha = 1, aes(fill = metadocencia), color = "gray", size = 0.1) +
  labs(title = paste0("Ya demostraron interés ", interes, " docentes de ", length(paises_metadocencia), " paises"),
       subtitle = "",
       x = "",
       y = "") +
...
```{r mapaargentina, echo=FALSE, message=FALSE, warning=FALSE, fig.align="left", fig.width=9}
ggplot() +
  geom_sf(data = mapaargentina, alpha = 1, aes(fill = cantidad), color = "gray") +
  labs(title = paste0("Ya participaron docentes de ", length(provincias_argentinas) - 1, " provincias argentinas además de CABA") ,
       x = "",
       y = "") +
  ...
  coord_sf(ylim = c(-55,-21), xlim = c(-74, -50), expand = FALSE)  +
  theme_map() + 
  theme(plot.title = element_text(face = "bold",family = "Arial Rounded MT Bold", hjust = 0),
